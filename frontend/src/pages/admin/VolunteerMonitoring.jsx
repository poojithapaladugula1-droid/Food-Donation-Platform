...
import { IconTrophy, IconAward, IconStar, IconTruck, IconClock, IconMedal, IconRefresh, IconEye } from '@tabler/icons-react'
...
const VolunteerMonitoring = () => {
  ...
  const getLevelInfo = (points) => {
    if (points >= 2000) return { name: 'Platinum', color: '#e5e4e2', minPoints: 2000 }
    if (points >= 1000) return { name: 'Gold', color: '#ffd700', minPoints: 1000 }
    if (points >= 500) return { name: 'Silver', color: '#c0c0c0', minPoints: 500 }
    return { name: 'Bronze', color: '#cd7f32', minPoints: 0 }
  }
              ...
              <Group justify="space-between" align="flex-start">
                <Group>
                  ...
                  <div>
                    ...
                    <Badge size="md" color={index === 0 ? 'gold' : index === 1 ? 'gray' : 'orange'}>
            ...
            {filteredVolunteers.map((volunteer) => {
              ...
              return (
                <Table.Tr key={volunteer._id}>
                  ...
                  <Table.Td>
                    <Group gap="xs">
                      <IconAward size={14} color="#22c55e" />
                      {volunteer.points}
                    </Group>
                  </Table.Td>
                  <Table.Td>
                  ...
                  <Table.Td>
                    <Badge color={volunteer.status === 'active' ? 'green' : 'red'}>
          ...
          <Stack gap="md">
            <Group>
              ...
              <div>
                ...
                <Badge color="blue" mt="xs">Member since {new Date(selectedVolunteer.joinedDate).toLocaleDateString()}</Badge>
              </div>
            </Group>
            ...
            <div>
              <Text fw={600} mb="xs">Recent Deliveries</Text>
              {selectedVolunteer.recentDeliveries?.length > 0 ? (
                <Table>
                  <Table.Thead>
                    <Table.Tr>
                  ...
                  <Table.Tbody>
                    {selectedVolunteer.recentDeliveries.map((delivery, i) => (
                      <Table.Tr key={i}>
                        <Table.Td>{delivery.foodName}</Table.Td>
                        <Table.Td>{new Date(delivery.completedAt).toLocaleDateString()}</Table.Td>
                        <Table.Td>+{delivery.points}</Table.Td>
                      </Table.Tr>
                    ))}
                  </Table.Tbody>
                </Table>
              ) : (
                <Text c="dimmed" ta="center" py="md">No recent deliveries</Text>
              )}
            </div>

            <div>
              <Text fw={600} mb="xs">Badges Earned</Text>
              <Group gap="sm">
                {selectedVolunteer.badges?.map((badge, i) => (
                  <Badge key={i} size="lg" variant="light" color="yellow">
                    {badge}
                  </Badge>
                ))}
                {(!selectedVolunteer.badges || selectedVolunteer.badges.length === 0) && (
                  <Text c="dimmed" size="sm">No badges earned yet</Text>
                )}
              </Group>
            </div>
          </Stack>
